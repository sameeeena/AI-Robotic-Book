...
class FaissService:
    def __init__(self, dimension: int = 1536, index_path: str = None, payload_path: str = None):
        # Use /tmp for Vercel compatibility if paths not provided
        if index_path is None:
            index_path = os.path.join("/tmp", "faiss_index.bin")
        if payload_path is None:
            payload_path = os.path.join("/tmp", "payloads.pkl")
    ...
    def _save(self):
        try:
            # Ensure directory exists (mostly for local testing if not using /tmp)
